...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:SqlDataSource ID="BaddieTable" runat="server" ConflictDetection="CompareAllValues" ConnectionString='<%$ ConnectionStrings:UserAccountCS %>' DeleteCommand="DELETE FROM [Baddies] WHERE [Id] = @original_Id AND (([Name] = @original_Name) OR ([Name] IS NULL AND @original_Name IS NULL)) AND (([Type] = @original_Type) OR ([Type] IS NULL AND @original_Type IS NULL)) AND (([Health] = @original_Health) OR ([Health] IS NULL AND @original_Health IS NULL)) AND (([Damage] = @original_Damage) OR ([Damage] IS NULL AND @original_Damage IS NULL)) AND (([Speed] = @original_Speed) OR ([Speed] IS NULL AND @original_Speed IS NULL))" InsertCommand="INSERT INTO [Baddies] ([Name], [Type], [Health], [Damage], [Speed]) VALUES (@Name, @Type, @Health, @Damage, @Speed)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [Baddies]" UpdateCommand="UPDATE [Baddies] SET [Name] = @Name, [Type] = @Type, [Health] = @Health, [Damage] = @Damage, [Speed] = @Speed WHERE [Id] = @original_Id AND (([Name] = @original_Name) OR ([Name] IS NULL AND @original_Name IS NULL)) AND (([Type] = @original_Type) OR ([Type] IS NULL AND @original_Type IS NULL)) AND (([Health] = @original_Health) OR ([Health] IS NULL AND @original_Health IS NULL)) AND (([Damage] = @original_Damage) OR ([Damage] IS NULL AND @original_Damage IS NULL)) AND (([Speed] = @original_Speed) OR ([Speed] IS NULL AND @original_Speed IS NULL))">
        <DeleteParameters>
    ...
    <asp:SqlDataSource ID="SelectedBaddie" runat="server" ConnectionString='<%$ ConnectionStrings:UserAccountCS %>' SelectCommand="SELECT * FROM [Baddies] WHERE ([Id] = @Id)">
        <SelectParameters>
